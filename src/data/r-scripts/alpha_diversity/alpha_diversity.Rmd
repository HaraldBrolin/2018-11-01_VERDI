...
```{r}
df <- readRDS(here("qiime_to_phyloseq/physeq.rds")) # Read the data frame
```
...
The article mentions **Shannon** and **Simpson**:grid.arrange(p3,p4)
...
richness_df$sample_depth <- # Add depth to metadata, for each sample
  df@otu_table %>%
  colSums()
...
p7 <-  # Plot longitudinal scatter,ordered based on sampling_date
  ggplot(richness_df,aes(x = fs, y = Simpson, group = fs)) +
  facet_wrap(~period_veg) + # One plot for each location
